...
const handler = async (req, res) => {
  if (req.method === 'POST') {
    ...
    uploadHandler(req, res, async (err) => {
      ...
      if (!id || !imageKey) {
        return res.status(400).json({ error: 'ID de la visita técnica o key de la imagen no proporcionados' });
      }
      ...
      try {
        ...
        console.error('Error al actualizar la visita técnica:', dbError);
        return res.status(500).json({ error: 'Error al actualizar la visita técnica', details: dbError.message });
      }
    });
    ...
    try {
      ...
      if (result.affectedRows === 0) {
        return res.status(404).json({ error: 'No se pudo actualizar la visita técnica' });
      }
